                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($courses as $course)
                        <div class="card p-5">
                            ...
                            <h3 class="mt-3 text-sm font-semibold text-slate-900">{{ $course->title }}</h3>
                            <p class="mt-3 text-2xl font-extrabold text-slate-900">{{ $course->priceFormatted() }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    @foreach ($robots as $robot)
                        <div class="card p-5">
                            <div class="flex items-center gap-3">
                                ...
                                <h3 class="font-semibold text-slate-900">{{ $robot->name }}</h3>
                            </div>
                            <div class="mt-4 flex items-end justify-between">
                                <p class="text-2xl font-extrabold text-slate-900">{{ $robot->priceFormatted() }}</p>
                                <span class="text-xs text-slate-500">/ {{ $robot->duration_days }} days</span>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    @foreach ($mentorships as $m)
                        <div class="card p-5">
                            ...
                            <p class="mt-4 text-2xl font-extrabold text-slate-900">{{ $m->priceFormatted() }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
