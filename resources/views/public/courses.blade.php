<x-layouts.public title="Courses — EMMIOXFOREX ACADEMY">
    ...
    <section class="px-4 py-16 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl">
            ...
            @php $allCourses = $courses->flatten(); @endphp
                ...
                <div id="course-grid" class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($allCourses as $course)
                        <a href="{{ route('courses.show', $course) }}"
                           ...
                           data-desc="{{ strtolower($course->description) }}">
                            ...
                            @endif
                                ...
                                {{-- Level badge --}}
                                <span class="inline-flex self-start items-center px-2.5 py-0.5 rounded-full text-xs font-semibold mb-3 badge-level-{{ $course->level }}">
                                    {{ ucfirst($course->level) }}
                                </span>

                                <h3 class="font-bold text-slate-900 transition group-hover:text-brand-600 leading-snug">{{ $course->title }}</h3>
                                <p class="mt-2 flex-1 line-clamp-2 text-sm leading-relaxed text-slate-500">{{ $course->description }}</p>
