<x-layouts.app :title="($title ?? 'Dashboard').' — EMMIOXFOREX ACADEMY'">
@php
    $authUser        = auth()->user();
    $memberNoteCount = $authUser->payments()
        ...
        ->count();
@endphp
    {{-- ── h-screen wrapper: sidebar + content side-by-side, no page scroll ── --}}
    <div class="flex h-screen overflow-hidden bg-slate-50">

        {{-- ── Sidebar (dark navy, full height, internal nav scrolls) ── --}}
        ...
        {{-- ── Right column: mobile bars (fixed) + scrollable content ── --}}
        <div class="flex flex-1 flex-col min-w-0 overflow-hidden">

            {{-- Mobile top bar (always visible, never scrolls) --}}
            ...
            {{-- Mobile nav drawer (never scrolls page) --}}
            <div id="mob-member-menu" class="hidden flex-shrink-0 border-b border-slate-200 bg-white px-4 pb-4 lg:hidden">
                <nav class="mt-3 space-y-1">
                    <a href="{{ route('member.dashboard') }}"        class="{{ request()->routeIs('member.dashboard')     ? 'nav-link-active' : 'nav-link' }}">Overview</a>
                    <a href="{{ route('member.courses.index') }}"    class="{{ request()->routeIs('member.courses.*')     ? 'nav-link-active' : 'nav-link' }}">My Courses</a>
                    <a href="{{ route('member.robots.index') }}"     class="{{ request()->routeIs('member.robots.*')      ? 'nav-link-active' : 'nav-link' }}">Robots / EAs</a>
                    <a href="{{ route('member.signals.index') }}"    class="{{ request()->routeIs('member.signals.*')     ? 'nav-link-active' : 'nav-link' }}">Signals</a>
                    <a href="{{ route('member.mentorship.index') }}" class="{{ request()->routeIs('member.mentorship.*') ? 'nav-link-active' : 'nav-link' }}">Mentorship</a>
                    <a href="{{ route('member.billing.index') }}"    class="{{ request()->routeIs('member.billing.*')     ? 'nav-link-active' : 'nav-link' }}">Billing</a>
                </nav>
            </div>
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                {{-- Desktop sticky topbar (sticks to top of THIS scroll container) --}}
                <div class="sticky top-0 z-30 hidden items-center justify-between border-b border-slate-200 bg-white px-6 py-3 lg:flex">
                    <div class="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        @if(isset($header))
                            ...
                            <span>{{ is_string($header) ? $header : '' }}</span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="relative" id="member-notif-wrapper">
                            <button id="member-notif-btn"
                                ...
                                @if($memberNoteCount > 0)
                                    <span class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-[9px] font-bold text-white">{{ $memberNoteCount > 9 ? '9+' : $memberNoteCount }}</span>
                                @endif
                            </button>
                        ...
                        <div class="relative" id="member-avatar-menu">
                            <button id="member-avatar-btn"
                                ...
                                <span class="flex h-7 w-7 items-center justify-center rounded-full bg-brand-500/20 text-xs font-bold text-brand-600">{{ strtoupper(substr($authUser->name, 0, 1)) }}</span>
                                <span class="hidden max-w-[120px] truncate font-medium text-slate-700 sm:block">{{ explode(' ', $authUser->name)[0] }}</span>
                            ...
                            <div id="member-avatar-dropdown"
                                ...
                                <a href="{{ route('member.billing.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 transition hover:bg-slate-50 hover:text-brand-600">
                                ...
                                <a href="{{ route('member.courses.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 transition hover:bg-slate-50 hover:text-brand-600">
