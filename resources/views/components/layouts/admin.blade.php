<x-layouts.app :title="($title ?? 'Admin').' — Admin Panel'">
    {{-- ── h-screen wrapper: sidebar never scrolls, only content does ── --}}
        ...
        <aside class="hidden w-64 flex-shrink-0 flex-col bg-navy-950 lg:flex">
            ...
            <nav class="flex-1 space-y-0.5 overflow-y-auto px-3 py-5">
                ...
                <a href="{{ route('admin.notifications') }}"
                   class="{{ request()->routeIs('admin.notifications') ? 'sidebar-link-active' : 'sidebar-link' }} justify-between">
                    <span class="flex items-center gap-3">
                        <svg class="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                        Notifications
                    </span>
                    @php $totalPending = ($pendingPaymentsCount ?? 0) + ((\App\Models\User::where('role','member')->where('status','pending')->count()) ?? 0); @endphp
                    @if($totalPending > 0)
                        <span class="rounded-full bg-gold-500 px-1.5 py-0.5 text-[10px] font-bold text-white">{{ $totalPending }}</span>
                    @endif
                </a>

                <a href="{{ route('admin.contact.index') }}"
                   class="{{ request()->routeIs('admin.contact.*') ? 'sidebar-link-active' : 'sidebar-link' }} justify-between">
                    <span class="flex items-center gap-3">
                    ...
                    @php $contactUnread = \App\Models\ContactMessage::where('status','new')->count(); @endphp
                    @if($contactUnread > 0)
                        <span class="rounded-full bg-brand-500 px-1.5 py-0.5 text-[10px] font-bold text-white">{{ $contactUnread }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('admin.testimonials.index') }}"
                   class="{{ request()->routeIs('admin.testimonials.*') ? 'sidebar-link-active' : 'sidebar-link' }}">
                ...
                <a href="{{ route('admin.payment-methods.index') }}"
                   class="{{ request()->routeIs('admin.payment-methods.*') ? 'sidebar-link-active' : 'sidebar-link' }}">
                    <svg class="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
                    Payment Methods
                </a>
        ...
        <div class="flex flex-1 flex-col min-w-0 overflow-hidden">
            ...
            <div id="mob-admin-menu" class="hidden flex-shrink-0 border-b border-slate-200 bg-white px-4 pb-4 lg:hidden">
                <nav class="mt-3 space-y-1">
                    ...
                    <a href="{{ route('admin.contact.index') }}"  class="{{ request()->routeIs('admin.contact.*')    ? 'nav-link-active' : 'nav-link' }}">Contact</a>
                </nav>
            </div>
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('admin.notifications') }}"
                           class="relative flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-700">
                            ...
                            @if(($pendingPaymentsCount ?? 0) > 0)
                                <span class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-rose-500 text-[9px] font-bold text-white">{{ $pendingPaymentsCount > 9 ? '9+' : $pendingPaymentsCount }}</span>
                            @endif
                        </a>
                        <div class="relative" id="admin-avatar-menu">
                            <button id="admin-avatar-btn"
                                ...
                                <span class="flex h-7 w-7 items-center justify-center rounded-full bg-gold-500/20 text-xs font-bold text-gold-600">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                                <span class="hidden max-w-[120px] truncate font-medium text-slate-700 sm:block">{{ auth()->user()->name }}</span>
                            ...
                            <div id="admin-avatar-dropdown"
                                ...
                                <a href="{{ route('admin.profile') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-brand-600">
                                ...
                                <a href="{{ route('admin.preferences') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-brand-600">
                                ...
                                <a href="{{ route('admin.pricing') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-brand-600">
