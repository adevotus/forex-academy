<x-layouts.admin title="Pricing">
    ...
    <div class="mb-8 flex items-center justify-between rounded-xl border border-slate-200 bg-white px-5 py-3 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="rounded-full bg-brand-50 px-3 py-0.5 text-sm font-bold text-brand-700">{{ $currency }}</span>
                @if($currency === 'TZS')
                    <span class="text-xs text-slate-400">· 1 USD = TZS {{ number_format($rate, 0) }}</span>
                @endif
            </div>
        </div>
        <a href="{{ route('admin.preferences') }}" class="btn-outline !py-1.5 text-xs">
    ...
    <div class="space-y-8">
        ...
        <div class="card overflow-hidden">
            ...
            <form method="POST" action="{{ route('admin.pricing.settings') }}" class="p-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div class="relative mt-1">
                            ...
                            <input id="registration_fee" type="number" name="registration_fee" step="0.01" min="0"
                                   value="{{ $settings['registration_fee'] }}" class="input pl-7">
                        </div>
                        @if($currency === 'TZS')
                            <p class="mt-1.5 text-xs font-medium text-brand-600">≈ {{ $fmt($settings['registration_fee']) }}</p>
                        @endif
                        @error('registration_fee')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div class="relative mt-1">
                            ...
                            <input id="signal_price" type="number" name="signal_price" step="0.01" min="0"
                                   value="{{ $settings['signal_price'] }}" class="input pl-7">
                        </div>
                        @if($currency === 'TZS')
                            <p class="mt-1.5 text-xs font-medium text-brand-600">≈ {{ $fmt($settings['signal_price']) }}</p>
                        @endif
                        @error('signal_price')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
                    </div>

                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <label class="label text-xs" for="usd_to_tzs">USD → TZS Rate</label>
                        <input id="usd_to_tzs" type="number" name="usd_to_tzs" step="1" min="1"
                               value="{{ $settings['usd_to_tzs'] }}" class="input mt-1">
                        <p class="mt-1.5 text-xs text-slate-400">Rate per $1 USD</p>
                        @error('usd_to_tzs')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <select id="currency" name="currency" class="input mt-1">
                            ...
                            <option value="TZS" {{ $settings['currency'] === 'TZS' ? 'selected' : '' }}>TZS — Shilling</option>
                        </select>
                        @error('currency')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
                    </div>
        ...
        <div class="card overflow-hidden">
            ...
            @else
                <form method="POST" action="{{ route('admin.pricing.robots') }}" class="p-6">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($robots as $robot)
                            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                                <div class="mb-3">
                                    <p class="truncate font-semibold text-slate-900 text-sm">{{ $robot->name }}</p>
                                    <p class="text-xs text-slate-500 mt-0.5">v{{ $robot->version }} · {{ $robot->duration_days }} days</p>
                                </div>
                                <div class="relative">
                                    <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-400 text-sm">$</span>
                                    <input type="number" name="prices[{{ $robot->id }}]"
                                           value="{{ number_format((float)$robot->price, 2, '.', '') }}"
                                           step="0.01" min="0" class="input pl-7 py-2 text-sm">
                                </div>
                                @if($currency === 'TZS')
                                    <p class="mt-1.5 text-xs font-medium text-brand-600">≈ {{ $fmt($robot->price) }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div class="card overflow-hidden">
                ...
                <div class="p-6">
                    <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-5">
                        ...
                        <p class="mt-1 text-3xl font-extrabold text-emerald-700">${{ number_format((float)$settings['signal_price'], 2) }}</p>
                        @if($currency === 'TZS')
                            <p class="mt-1 text-xs font-medium text-slate-600">≈ {{ $fmt($settings['signal_price']) }}</p>
                        @endif
            ...
            <div class="card overflow-hidden">
                ...
                @else
                    <form method="POST" action="{{ route('admin.pricing.mentorship') }}" class="p-6">
                        @csrf
                        @method('PUT')
                        <div class="space-y-3">
                            @foreach($mentorships as $m)
                                <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                                    <div class="flex-1 min-w-0">
                                        <p class="truncate text-sm font-semibold text-slate-900">{{ $m->title }}</p>
                                        @if($currency === 'TZS')
                                            <p class="text-xs font-medium text-brand-600 mt-0.5">≈ {{ $fmt($m->price) }}</p>
                                        @endif
                                    </div>
                                    <div class="relative w-32 flex-shrink-0">
                                        <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-400 text-sm">$</span>
                                        <input type="number" name="prices[{{ $m->id }}]"
                                               value="{{ number_format((float)$m->price, 2, '.', '') }}"
                                               step="0.01" min="0" class="input pl-7 py-2 text-sm">
                                    </div>
                                </div>
                            @endforeach
