...
<div class="mx-auto max-w-3xl">
            ...
            <div class="flex flex-1 items-center">
                <div class="flex flex-col items-center flex-shrink-0">
                    @if($step['done'])
                        ...
                        <span class="mt-1.5 text-[10px] font-medium text-center leading-tight text-emerald-600">{{ $step['label'] }}</span>
                    @elseif($step['current'] ?? false)
                        ...
                        <span class="mt-1.5 text-[10px] font-semibold text-center leading-tight text-slate-800">{{ $step['label'] }}</span>
                    @else
                        ...
                        <span class="mt-1.5 text-[10px] font-medium text-center leading-tight text-slate-400">{{ $step['label'] }}</span>
                    @endif
                </div>
                @if(! $loop->last)
                    <div class="mx-2 h-0.5 flex-1 {{ $step['done'] ? 'bg-emerald-400' : 'bg-slate-200' }} mb-4"></div>
                @endif
            </div>
        @endforeach
    </div>
    ...
    <div class="mb-8 rounded-2xl p-6 text-white shadow-xl" style="background: linear-gradient(135deg, #0a1628 0%, #0f2a4a 50%, #1a3a5c 100%)">
        ...
        <div class="mt-3 flex items-end gap-2">
            <span class="text-5xl font-extrabold text-white">{{ $registrationFee }}</span>
            <span class="mb-1.5 text-xl font-semibold" style="color:rgba(203,213,225,0.9)">{{ $currency }}</span>
        </div>
        <p class="mt-1 text-sm" style="color:rgba(148,163,184,0.85)">Paid once. Lifetime academy membership.</p>

        <div class="mt-4 flex flex-wrap gap-2">
            @foreach(['Courses', 'Robots', 'Signals', 'Mentorship'] as $feature)
    ...
    {{-- ── Two-column: payment methods + upload ── --}}
            ...
            @else
                <div class="space-y-3">
                    ...
                    @foreach($paymentMethods as $pm)
                        @php $iconStyle = $iconStyleMap[$pm->icon_color] ?? $iconStyleMap['slate']; @endphp
                        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <div class="flex items-center gap-3 mb-3">
                                <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl text-sm font-extrabold" style="{{ $iconStyle }}">
                                    {{ $pm->typeIcon() }}
                                </div>
                                <div>
                                    <p class="text-sm font-semibold text-slate-900">{{ $pm->name }}</p>
                                    @if($pm->subtitle)
                                        <p class="text-xs text-slate-400">{{ $pm->subtitle }}</p>
                                    @endif
                                </div>
                            </div>
                            @if(!empty($pm->details))
                                <dl class="space-y-2 rounded-lg bg-slate-50 p-3">
                                    @foreach($pm->details as $detail)
                                        @php $isLong = strlen($detail['value']) > 20; @endphp
                                        @if($isLong)
                                            {{-- Stacked layout for long values (wallet addresses etc.) --}}
                                            <div class="text-xs">
                                                <dt class="text-slate-400 font-medium mb-1">{{ $detail['label'] }}</dt>
                                                <dd class="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2">
                                                    <span class="font-mono font-semibold text-slate-800 text-[11px] break-all flex-1 leading-relaxed">{{ $detail['value'] }}</span>
                                                    <button type="button"
                                                        onclick="copyToClipboard(this, '{{ addslashes($detail['value']) }}')"
                                        ...
                                        @else
                                            ...
                                            <div class="flex items-center justify-between text-xs">
                                                <dt class="text-slate-400 font-medium">{{ $detail['label'] }}</dt>
                                                <dd class="font-semibold text-slate-800">{{ $detail['value'] }}</dd>
                                            </div>
                                        @endif
                                    @endforeach
                                </dl>
                    ...
                    {{-- Reference note --}}
                    @php $noteMethod = $paymentMethods->firstWhere('note', '!=', null); @endphp
                    @if($noteMethod)
                        <div class="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-slate-700 leading-relaxed">
                            ...
                            Use <span class="font-semibold text-slate-900 bg-amber-100 px-1 rounded">{{ $user->email }}</span> as your payment reference.
                        </div>
                    @endif
                </div>
            @endif
            ...
            @if($proofSubmitted)
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <details class="mt-2 group">
                        ...
                        <div class="mt-3">
                            @include('member.partials.proof-upload-form')
                        </div>
                    </details>
                </div>
