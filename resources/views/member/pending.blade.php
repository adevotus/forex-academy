...
<header class="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur-sm">
    <div class="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-3">
            <span class="hidden text-xs text-slate-400 sm:block">{{ auth()->user()->email }}</span>
...
<main class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    {{-- ── Progress steps — update based on $currentStep ────── --}}
    <div class="mb-10 flex items-center justify-center">
        ...
        {{-- Connector 1→2 --}}
        <div class="mx-3 h-px w-10 sm:w-20 {{ $currentStep >= 2 ? 'bg-brand-300' : 'bg-slate-200' }}"></div>

        {{-- Step 2: active when currentStep=2, done when currentStep≥3 --}}
        <div class="flex items-center gap-2">
            @if($currentStep >= 3)
                <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
        ...
        {{-- Connector 2→3 --}}
        <div class="mx-3 h-px w-10 sm:w-20 {{ $currentStep >= 3 ? 'bg-brand-300' : 'bg-slate-200 opacity-40' }}"></div>

        {{-- Step 3: active when currentStep=3, else pending --}}
        <div class="flex items-center gap-2 {{ $currentStep >= 3 ? '' : 'opacity-40' }}">
            @if($currentStep >= 3)
                <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-brand-600 text-white shadow ring-4 ring-brand-100">
    ...
    {{-- ── Step 3: Proof received banner (only shown in step 3) ── --}}
    @if($currentStep === 3)
        ...
        <div class="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <div class="mt-2 flex items-baseline gap-3">
                    <span class="text-6xl font-black tracking-tight leading-none">{{ $currencySymbol }}{{ number_format($regFee, 0) }}</span>
                    <span class="text-xl font-semibold text-brand-200">{{ $currency }}</span>
                </div>
                <p class="mt-3 text-sm text-brand-200">Paid once. Lifetime academy membership.</p>
                <div class="mt-4 flex flex-wrap gap-3">
                    @foreach(['Courses', 'Robots', 'Signals', 'Mentorship'] as $perk)
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-5">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4 flex items-start gap-2 rounded-xl border border-amber-100 bg-amber-50 p-3">
                    ...
                    <p class="text-xs leading-relaxed text-amber-700">
                        Use <strong class="font-bold">{{ auth()->user()->email }}</strong> as your payment reference so we can match your payment quickly.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-3">
            <div class="h-full rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                @if($currentStep === 3)
                    {{-- Step 3: show awaiting state + allow re-upload --}}
                    ...
                    {{-- Allow re-upload in case they sent the wrong file --}}
                    <details class="group rounded-xl border border-slate-200 bg-slate-50">
                        ...
                        <div class="border-t border-slate-200 p-4">
                            ...
                            @endif
                            <form method="POST" action="{{ route('member.pending.proof') }}"
            ...
            @foreach([
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm">
                    <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl
                        @if($color==='emerald') bg-emerald-50 text-emerald-600 ring-1 ring-emerald-100
                        @elseif($color==='amber') bg-amber-50 text-amber-600 ring-1 ring-amber-100
                        @elseif($color==='brand') bg-brand-50 text-brand-600 ring-1 ring-brand-100
                        @else bg-violet-50 text-violet-600 ring-1 ring-violet-100 @endif">
