        ...
        @foreach ($robots as $robot)
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col transition hover:shadow-md hover:-translate-y-0.5">
                ...
                <div class="relative h-40 overflow-hidden">
                    ...
                    @else
                        {{-- Default placeholder: dark tech gradient with decorative SVG --}}
                        <div class="h-full w-full flex items-center justify-center relative"
                            ...
                            <svg class="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 400 160" preserveAspectRatio="xMidYMid slice">
                                <defs>
                                    <pattern id="grid-{{ $robot->id }}" width="40" height="40" patternUnits="userSpaceOnUse">
                                        <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#60a5fa" stroke-width="0.5"/>
                                    </pattern>
                                </defs>
                                <rect width="400" height="160" fill="url(#grid-{{ $robot->id }})"/>
                                {{-- Glow circle --}}
                ...
                <div class="flex flex-1 flex-col p-4">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <h3 class="truncate text-base font-bold text-slate-900">{{ $robot->name }}</h3>
                            <p class="text-[11px] text-slate-400">v{{ $robot->version }}</p>
                        </div>
                    </div>
                    <p class="mt-2 flex-1 text-xs leading-relaxed text-slate-500 line-clamp-2">{{ $robot->description }}</p>
                    ...
                    <div class="mt-4 space-y-2">
                        @if ($sub)
                            ...
                            <a href="{{ route('member.robots.show', $robot) }}"
                        ...
                        @else
                            <div class="flex items-end justify-between">
                                <div>
                                    <p class="text-xl font-extrabold text-slate-900">{{ $robot->priceFormatted() }}</p>
                                    <p class="text-[10px] text-slate-400">{{ $robot->duration_days }}-day access</p>
                                </div>
                            </div>
                            <button type="button"
