    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        @foreach ($robots as $robot)
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <h3 class="mt-4 text-lg font-semibold text-slate-900">{{ $robot->name }}</h3>
                <p class="mt-1 text-xs text-slate-400">Version {{ $robot->version }}</p>
                <p class="mt-3 text-sm leading-relaxed text-slate-500">{{ $robot->description }}</p>
                @if ($sub)
                    <p class="mt-4 text-xs text-slate-400">Expires {{ $sub->expires_at?->format('M d, Y') }}</p>
                ...
                @else
                    <p class="mt-4 text-xl font-bold text-slate-900">{{ $robot->priceFormatted() }}</p>
                    <button type="button"
