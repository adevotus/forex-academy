        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span class="text-xs text-slate-400"><?php echo e($unlockedCourses->count()); ?> course<?php echo e($unlockedCourses->count() !== 1 ? 's' : ''); ?></span>
                </div>
                <?php if($unlockedCourses->isEmpty()): ?>
                    <div class="px-5 py-8 text-center text-sm text-slate-400">No courses unlocked yet.</div>
                <?php else: ?>
                    <div class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $unlockedCourses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="px-5 py-3 flex items-center gap-4">
                                <?php if($item['course']->cover_image): ?>
                                    <img src="<?php echo e(asset('storage/'.$item['course']->cover_image)); ?>"
                                ...
                                <?php endif; ?>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-slate-900 truncate"><?php echo e($item['course']->title); ?></p>
                                    <p class="text-xs text-slate-400 mt-0.5">
                                        <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600 mr-1"><?php echo e(ucfirst($item['course']->level)); ?></span>
                                        Unlocked <?php echo e($item['unlocked_at']?->format('M j, Y') ?? '—'); ?>

                                    </p>
                                </div>
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    <span class="text-sm font-bold text-slate-800"><?php echo e($item['amount']); ?></span>
